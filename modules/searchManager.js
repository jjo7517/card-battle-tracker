...
const SearchManager = (function () {
    ...
    function updateSearchOptions() {
        ...
        if (customSearchContainer) {
            if (customFields.length > 0) {
                    ...
                    <div class="search-grid">
                        ${customFields.map(field => `
                            <div class="form-group">
                                <label>🏷️ ${DataManager.escapeHtml(field.name)}</label>
                                <div class="btn-group cf-btn-group" id="search-${field.id}" data-value="">
                                    <button type="button" class="btn-group-item cf-btn cf-none active" data-value="">—</button>
                                    <button type="button" class="btn-group-item cf-btn cf-circle" data-value="○">○</button>
                                    <button type="button" class="btn-group-item cf-btn cf-cross" data-value="✕">✕</button>
                                    <button type="button" class="btn-group-item cf-btn cf-triangle" data-value="△">△</button>
                                    <input type="hidden" id="search-${field.id}-val" value="">
                                </div>
                            </div>
                        `).join('')}
                    </div>
                `;
                // 綁定按鈕組事件
                customSearchContainer.querySelectorAll('.cf-btn-group').forEach(group => {
    ...
    function getSearchFilters() {
        ...
        customFields.forEach(field => {
            const hidden = document.getElementById(`search-${field.id}-val`);
            if (hidden && hidden.value) {
                filters.customFields[field.id] = hidden.value;
            }
        });
    ...
    function resetSearch() {
        ...
        // 重置自訂欄位按鈕組
        DataManager.getCustomFields().forEach(field => {
            const hidden = document.getElementById(`search-${field.id}-val`);
            if (hidden) hidden.value = '';
            const group = document.getElementById(`search-${field.id}`);
            if (group) {
                group.querySelectorAll('.btn-group-item').forEach(b => b.classList.remove('active'));
        ...
        if (tbody) {
            const html = pageRecords.map(record => {
                ...
                customFields.forEach(field => {
                    ...
                    customCells += `<td ${width}><span class="cf-badge ${badgeClass}">${escapeHtml(display)}</span></td>`;
                });

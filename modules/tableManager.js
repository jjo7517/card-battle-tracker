...
const TableManager = (function () {
    ...
    function bindEvents() {
        ...
        // Misplay Note Conditional Logic - 監聽按鈕組點擊
        const misplayGroup = document.getElementById('misplay-group');
        if (misplayGroup) {
            misplayGroup.addEventListener('click', updateMisplayNoteState);
    ...
    function getFormData() {
        ...
        // 取得自訂欄位值（從隱藏 input 讀取）
        customFields.forEach(field => {
            const element = document.getElementById(`field-${field.id}-val`);
            if (element) {
    ...
    function clearForm() {
        ...
        const customFields = DataManager.getCustomFields();
        customFields.forEach(field => {
            const hidden = document.getElementById(`field-${field.id}-val`);
            ...
            if (group) {
                ...
                const first = group.querySelector('.btn-group-item');
    ...
    function renderCustomFieldsForm() {
        ...
        const customFieldOptions = [
            { value: '', label: '—', cls: 'cf-none' },
            { value: '○', label: '○', cls: 'cf-circle' },
            { value: '✕', label: '✕', cls: 'cf-cross' },
            { value: '△', label: '△', cls: 'cf-triangle' }
        ];
                ...
                ${customFields.map(field => `
                    <div class="custom-field-item">
                        <div class="form-group">
                            <label>🏷️ ${escapeHtml(field.name)}</label>
                            <div class="btn-group cf-btn-group" id="field-${field.id}" data-value="">
                                ${customFieldOptions.map((opt, i) => `
                                    <button type="button" class="btn-group-item cf-btn ${opt.cls}${i === 0 ? ' active' : ''}" data-value="${opt.value}">${opt.label}</button>
                                `).join('')}
                                <input type="hidden" id="field-${field.id}-val" value="">
                            </div>
                        </div>
                        <button type="button" class="remove-field-btn" data-field-id="${field.id}" title="${i18n.get('confirm_remove_field')}">×</button>
                    </div>
                `).join('')}
            </div>
        `;
            ...
            <div class="custom-fields-grid">
                ${customFields.map(field => `
                    <div class="custom-field-item">
                        <div class="form-group">
                            <label>🏷️ ${escapeHtml(field.name)}</label>
                            <div class="btn-group cf-btn-group" id="edit-field-${field.id}" data-value="">
                                ${customFieldOptions.map((opt, i) => `
                                    <button type="button" class="btn-group-item cf-btn ${opt.cls}${i === 0 ? ' active' : ''}" data-value="${opt.value}">${opt.label}</button>
                                `).join('')}
                                <input type="hidden" id="edit-field-${field.id}-val" value="">
                            </div>
                        </div>
                    </div>
                `).join('')}
        ...
        // 綁定自訂欄位按鈕組點擊事件
        [container, editContainer].forEach(c => {
            if (!c) return;
            c.querySelectorAll('.cf-btn-group').forEach(group => {
                group.querySelectorAll('.btn-group-item').forEach(btn => {
                    btn.addEventListener('click', () => {
                        group.querySelectorAll('.btn-group-item').forEach(b => b.classList.remove('active'));
                        btn.classList.add('active');
                        const hidden = document.getElementById(group.id + '-val');
        ...
        */
        ...
        tbody.innerHTML = records.map(record => {
            ...
            customFields.forEach(field => {
                ...
                let badgeClass = 'cf-badge-none';
                if (value === '○') badgeClass = 'cf-badge-circle';
                else if (value === '✕') badgeClass = 'cf-badge-cross';
                else if (value === '△') badgeClass = 'cf-badge-triangle';
                const display = value || '—';
                customCells += `<td style="width: ${columnSettings.widths[field.id] || 'auto'}"><span class="cf-badge ${badgeClass}">${escapeHtml(display)}</span></td>`;
            });
    ...
    function openEditModal(id) {
        ...
        if (typeof window.setBtnGroup === 'function') {
            window.setBtnGroup('edit-misplay-group', record.misplay || '');
        } else {
        ...
        customFields.forEach(field => {
            ...
            const hidden = document.getElementById(`edit-field-${field.id}-val`);
            if (hidden) hidden.value = val;
            const group = document.getElementById(`edit-field-${field.id}`);
            if (group) {
                group.querySelectorAll('.btn-group-item').forEach(b => {
                    b.classList.toggle('active', b.dataset.value === val);
                });
                // 如果沒有匹配的，選第一個（未選擇）
                const anyActive = group.querySelector('.btn-group-item.active');
                if (!anyActive) {
                    const first = group.querySelector('.btn-group-item');
                    if (first) first.classList.add('active');
                }
    ...
    function confirmEdit() {
        ...
        customFields.forEach(field => {
            const hidden = document.getElementById(`edit-field-${field.id}-val`);
            if (hidden) {
                updatedData[field.id] = hidden.value;
            }
        });

...
const DataManager = (function () {
    ...
    function generateTestData(count = 1000) {
        const lang = (typeof i18n !== 'undefined' && i18n.getLang) ? i18n.getLang() : 'zh-TW';
        ...
        const turnOrders = ['先手', '後手', '被讓先'];
        const turnOrderWeights = [0.44, 0.44, 0.12]; // 被讓先較少
        const results = ['勝利', '敗北', '平手'];
        const resultWeights = [0.48, 0.45, 0.07];
        const misplays = ['', '無', '無', '輕度', '輕度', '中等', '嚴重']; // 加權：無和輕度較多
        const customFieldValues = ['', '○', '✕', '△', '○', '']; // 加權：空和○較多
        ...
        for (let i = 0; i < count; i++) {
            ...
            let turnIdx = 0;
            let cumTurnWeight = 0;
            for (let j = 0; j < turnOrderWeights.length; j++) {
                cumTurnWeight += turnOrderWeights[j];
                if (randTurn <= cumTurnWeight) { turnIdx = j; break; }
            }
            ...
            customFields.forEach(field => {
                customData[field.id] = customFieldValues[Math.floor(Math.random() * customFieldValues.length)];
            });
